...
// Opening the action popup acknowledges the toolbar warning once.
void browser.runtime.sendMessage({ type: MESSAGE_TYPES.popupOpened }).catch(() => {});
...
const LIST_SVG = `<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path fill="currentColor" d="M40 48C26.7 48 16 58.7 16 72l0 48c0 13.3 10.7 24 24 24l48 0c13.3 0 24-10.7 24-24l0-48c0-13.3-10.7-24-24-24L40 48zM192 64c-17.7 0-32 14.3-32 32s14.3 32 32 32l288 0c17.7 0 32-14.3 32-32s-14.3-32-32-32L192 64zm0 160c-17.7 0-32 14.3-32 32s14.3 32 32 32l288 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-288 0zm0 160c-17.7 0-32 14.3-32 32s14.3 32 32 32l288 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-288 0zM16 232l0 48c0 13.3 10.7 24 24 24l48 0c13.3 0 24-10.7 24-24l0-48c0-13.3-10.7-24-24-24l-48 0c-13.3 0-24 10.7-24 24zM40 368c-13.3 0-24 10.7-24 24l0 48c0 13.3 10.7 24 24 24l48 0c13.3 0 24-10.7 24-24l0-48c0-13.3-10.7-24-24-24l-48 0z"/></svg>`;
const CHECK_SVG = `<svg viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path fill="currentColor" d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/></svg>`;
import {
  formatTimestamp,
  getStatusLabel,
  getExpiryTimestamp,
...
// History item id whose list-membership panel is open (one at a time). Tracked
...
document.addEventListener("click", (event) => {
  ...
  const listButton = event.target.closest("[data-action='toggle-list-panel']");
  if (listButton) {
    event.preventDefault();
    void handleToggleListPanel(listButton);
    return;
  }
...
// The list/favourite controls only make sense once an item has a Karakeep
// bookmark id (archived or skipped-duplicate).
function hasBookmark(item) {
  return (
    (item.status === HISTORY_STATUS.archived ||
      item.status === HISTORY_STATUS.skipped) &&
    Boolean(item.bookmarkId)
  );
}
...
function renderListButton(item) {
  ...
  const label = "Add to lists";
  return `
    <button
      type="button"
      class="list-button"
      data-action="toggle-list-panel"
      data-item-id="${escapeAttribute(item.id)}"
      data-bookmark-id="${escapeAttribute(item.bookmarkId)}"
      aria-label="${label}"
      data-tooltip="${label}"
...
// Empty, hidden container rendered inline with the row; populated on demand when
// the list button is toggled. Carries the data the panel needs (item/bookmark id
// and the current favourite state for the synthetic "Favourites" entry).
function renderListPanelContainer(item) {
  if (!hasBookmark(item)) {
    return "";
  }
  return `<div class="list-panel" hidden data-item-id="${escapeAttribute(item.id)}" data-bookmark-id="${escapeAttribute(item.bookmarkId)}" data-favourited="${item.favourited ? "true" : "false"}"></div>`;
}
...
async function handleToggleListPanel(button) {
  const row = button.closest(".item-row");
  const panel = row?.querySelector(".list-panel");
  if (!panel) {
    return;
  }
  const itemId = button.dataset.itemId;

  if (openListPanelItemId === itemId && !panel.hidden) {
    closeListPanel(panel);
    return;
  }
  // Only one membership panel open at a time.
  document.querySelectorAll(".list-panel").forEach((other) => {
    if (other !== panel) {
      other.hidden = true;
    }
  });
  openListPanelItemId = itemId;
  await openListPanel(panel, button.dataset.bookmarkId);
}

async function openListPanel(panel, bookmarkId) {
  panel.hidden = false;
  renderListPanel(panel, { loading: true });

  let response;
  try {
    response = await browser.runtime.sendMessage({
      type: MESSAGE_TYPES.fetchListsWithMembership,
      bookmarkId
    });
  } catch {
    response = null;
  ...
  // The popup may have re-rendered (replacing this panel) while we awaited.
  ...
  if (!response || response.ok === false || !Array.isArray(response.lists)) {
    renderListPanel(panel, {
      error: (response && response.message) || "Couldn't load lists"
    });
    return;
  }
  ...
  renderListPanel(panel, {
    ...
    memberListIds: Array.isArray(response.memberListIds)
      ? response.memberListIds
      : []
  });
}

// Favourites is always offered first; the manual lists (or a status note) follow.
function renderListPanel(panel, { loading, error, lists, memberListIds } = {}) {
  const itemId = panel.dataset.itemId;
  const bookmarkId = panel.dataset.bookmarkId;
  const isFavourited = panel.dataset.favourited === "true";

  const children = [buildFavouritesRow(itemId, bookmarkId, isFavourited)];
  if (loading) {
    children.push(panelNote("Loading…"));
  } else if (error) {
    children.push(panelNote(error));
  } else if (!lists || lists.length === 0) {
    children.push(panelNote("No lists yet"));
  } else {
    const members = new Set(memberListIds || []);
    for (const list of lists) {
      children.push(buildListRow(list, members.has(list.id), bookmarkId));
    }
  }
  panel.replaceChildren(...children);
}

// Reuses the existing toggle-favourite delegated handler so the panel star and
// the row star stay in sync.
function buildFavouritesRow(itemId, bookmarkId, isFavourited) {
  const row = document.createElement("button");
  row.type = "button";
  row.className = `list-row fav${isFavourited ? " is-member" : ""}`;
  row.dataset.action = "toggle-favourite";
  row.dataset.itemId = itemId;
  row.dataset.bookmarkId = bookmarkId;
  row.dataset.favourited = isFavourited ? "true" : "false";
  row.dataset.desiredState = isFavourited ? "false" : "true";
...
function buildListRow(list, isMember, bookmarkId) {
  const row = document.createElement("button");
  row.type = "button";
  row.className = `list-row${isMember ? " is-member" : ""}`;

  const icon = document.createElement("span");
  icon.className = "list-row-icon";
  const emoji = list.icon && String(list.icon).trim() ? String(list.icon).trim() : "";
  if (emoji) {
  ...
  const label = document.createElement("span");
  label.className = "list-row-label";
  label.textContent = list.name || "Untitled";
  ...
  row.addEventListener("click", () =>
    handleSetListMembership(row, list, bookmarkId, check)
  );
  return row;
}

async function handleSetListMembership(row, list, bookmarkId, check) {
  if (row.dataset.busy === "true") {
    return;
  }
  const wantMember = !row.classList.contains("is-member");
  row.dataset.busy = "true";
  try {
    const result = await browser.runtime.sendMessage({
      type: MESSAGE_TYPES.setListMembership,
      bookmarkId,
      listId: list.id,
      member: wantMember
    });
...
function panelNote(text) {
  ...
  el.className = "list-panel-note";
  el.textContent = text;
  return el;
}

function closeListPanel(panel) {
  if (panel) {
    panel.hidden = true;
  }
  openListPanelItemId = null;
}

// Re-open the membership panel after a re-render so a favourite toggle (which
// writes storage → debounced re-render) doesn't collapse it.
function restoreOpenListPanel() {
  if (!openListPanelItemId) {
    return;
  }
  let targetButton = null;
  document.querySelectorAll(".list-button").forEach((button) => {
    if (button.dataset.itemId === openListPanelItemId) {
      targetButton = button;
    }
  });
  if (!targetButton) {
    // Row no longer present (filtered out / expired) — drop the open state.
    openListPanelItemId = null;
    return;
  }
  const panel = targetButton.closest(".item-row")?.querySelector(".list-panel");
  if (panel) {
    void openListPanel(panel, targetButton.dataset.bookmarkId);
  }
}
